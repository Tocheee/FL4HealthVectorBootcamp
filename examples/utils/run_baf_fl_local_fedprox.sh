...
config_path="examples/baf_fedprox_example/config.yaml"

# dataset_path="/projects/federated_learning/RBC2/Bank_Account_Fraud/"
dataset_path="/projects/federated_learning/RBC2/share/"

server_output_file="examples/baf_fedprox_example/server.out"
client_output_folder="examples/baf_fedprox_example/"
scaler_save_path="examples/baf_fedprox_example/global_scaler.joblib"  # Adjust as needed
...
nohup python -m examples.baf_fedprox_example.server --config_path ${config_path} > ${server_output_file} 2>&1 &
...
for file in "${dataset_path}"/variant_*.csv; do
# for file in "${dataset_path}"/base*.csv; do
    ...
    data_file_path="$full_file_path" scaler_save_path="$scaler_save_path" nohup python -m examples.baf_fedprox_example.client > "$client_log_path" 2>&1 &
